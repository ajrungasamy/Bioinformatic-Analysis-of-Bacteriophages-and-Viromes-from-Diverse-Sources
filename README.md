...
This repository contains the Python scripts used for the computational analysis performed as part of an undergraduate dissertation project.
...
The script generates a genome summary table used for downstream analysis.
...
Combines the genome summary table with external metadata (for example host and lifestyle information) using accession identifiers or phage names as join keys.
...
* calculation of derived metrics (gene density and hypothetical protein fraction)
...
The scripts provided in this repository were used to analyse bacteriophage genome annotations derived from publicly available sequence databases. Due to dataset size, the genome input files and generated outputs are not included in this repository.
